...
del contexto compartido, y recién después lanza el resto en paralelo. Pero *quién*
ceba el caché se decide por la posición en la lista, no por si esa llamada
realmente ocurrió.

Si el primer auditor de la lista no tiene prompt, su ejecución retorna sin llamar
a la API: nadie escribe el caché y los cinco restantes salen en paralelo pagando
la escritura completa cada uno. El comportamiento está documentado y tiene un
test que lo demuestra; lo que falta es rediseñarlo para que el cebado dependa de
que `runAuditor` se haya invocado de verdad.
